...
import Data.Set hiding (map, foldl, filter, toList)
import Data.Maybe
import Data.Ix
import Debug.Trace
...
hasWon mat = (foldl (\prev curr -> prev + (fromEnum $ not (isUncovered $ coverage curr))) 0 (toList mat)) == bombCount
...
-- square has already been uncovered or a bomb has been detected in an adjacent square.
...
uncoverSquare (i, j) mat discovered = do
  ...
  let square = Square {
    bomb = bomp,
    coverage = if (bomp && (discovered /= empty)) then (coverage center) else Uncovered $ computeSurrounding i j mat
  }
  let new = setElem square (i, j) mat
  let newDiscover = insert (i, j) discovered

  if ((isntUncoverable center) || ((getSurrounding $ coverage square) /= 0)) then (new, empty) else (fst $ foldl (\(pmat, pset) (x, y) -> uncoverSquare (i+x, j+y) pmat pset)
    (new, newDiscover)
    (filter (\(x, y) ->
      and [
        (not $ isUncovered $ coverage center),
